...
public class ConsoleManager : MonoBehaviour
{
    ...
    void VerificarRespuesta(int NivelActual, int num)
    {
        ...
        if (NivelActual == 1)
        {
            respuestaCorrecta = Nivel1[num].RespuestaCorrecta;
        }
        else if (NivelActual == 2)
        {
            respuestaCorrecta = Nivel2[num].RespuestaCorrecta;
        }
        else if (NivelActual == 3)
        {
            respuestaCorrecta = Nivel3[num].RespuestaCorrecta;
        }

        Debug.Log($"Texto ingresado: '{textoIngresado}'");
        Debug.Log($"Respuesta correcta: '{respuestaCorrecta}'");

        if (textoIngresado == respuestaCorrecta)
        {
            contadorCorrectas++;
            ...
            consultaActual++;
            if (consultaActual >= 10)
            {
                if (NivelActual < 3)
                {
                    NivelActual++;
                    consultaActual = 0;
                    Numero = NumerosAleatorios(GetNivelArray(NivelActual).Length);
                }
                else
                {
                    Debug.Log("¡Se acabó el juego!");
                    // Puedes agregar lógica para finalizar el juego
                }
            }
    ...
    {
        Consulta consultaActual = new Consulta();
        Color color = Color.white;

        if (NivelActual == 1)
        {
            consultaActual = Nivel1[num];
            color = Color.white;
        }
        else if (NivelActual == 2)
        {
            consultaActual = Nivel2[num];
            color = new Color(0f, 27f, 178f, 1f);
        }
        else if (NivelActual == 3)
        {
            consultaActual = Nivel3[num];
            color = Color.black;
        }

        consola1.color = color;
        consola2.color = color;
        textConsulta.text = consultaActual.consulta;
        textConsola.text = consultaActual.IngresarTexto;
        Debug.Log($"Nivel {NivelActual} consulta: {consultaActual.consulta}");
        Debug.Log($"Nivel {NivelActual} texto inicial: {consultaActual.IngresarTexto}");
        Debug.Log($"Nivel {NivelActual} RESPUESTA: {consultaActual.RespuestaCorrecta}");
    }

    private Consulta[] GetNivelArray(int nivel)
    {
        if (nivel == 1) return Nivel1;
        if (nivel == 2) return Nivel2;
        if (nivel == 3) return Nivel3;
        return null;
    }
